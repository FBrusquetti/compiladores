...
{
		//strcpy(cad,t.lexema);
    //fprintf(archivo_salida,"%s%s%s","<",t.lexema,">");
		attribute();
	//	fprintf(archivo_salida,"%s%s%s","</",t.lexema,">");
...
int attribute()
{
  ...
  match("DOS_PUNTOS");
  attribute_value();
	fprintf(archivo_salida,"%s%s%s","</",name,">");
  return 0;
}

int attribute_name(char *name)
{

	char *name_sin_comillas = substr(t.lexema,1,(strlen(t.lexema)-2));
	fprintf(archivo_salida,"%s%s%s","<",name_sin_comillas,">");
	strcpy(name,name_sin_comillas);
  match("STRING");
  return 0;
}
...
int attribute_value()
{
  ...
  {
    match("STRING");
    fprintf(archivo_salida,"%s",cad_value);
  }
...
int main()
{
	...
	{
		archivo_salida=fopen("output.xml","a");
    		get_token();
		json();
